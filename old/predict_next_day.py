...
class stock_predictor():
    def __init__(self, params): 
        ...
        self.conn = sqlite3.connect('hidden_states_next_day.db')
    ...
    def run_decision_tree(self):
        ...
        self.train['target'] = self.train['close'].shift(-(self.hold_length+1)) / self.train['open'].shift(-1) - 1
        ...
        self.features = list(sfs.k_feature_names_)
        print('features', self.features, 'score', self.score)
    ...
    def get_trades(self):
        # get trades
        self.test['buy_price'] = self.test[self.trade_time].shift(-1)
        self.test['sell_price'] = self.test['close'].shift(-(self.hold_length+1))
        self.test['target'] = ( self.test['sell_price'] - self.test['buy_price'] ) / self.test['buy_price']
        self.test['buy_date'] = self.test['date'].shift(-1)
        self.test['sell_date'] = self.test['date'].shift(-(self.hold_length+1))
        self.test['trade_time'] = self.trade_time

        self.trades = self.test[ ['date', 'state', 'target', 'buy_date', 'sell_date', 'buy_price', 'sell_price'] ]
        self.trades['name'] = self.param_group_name
        
        

    def aggregate_results(self):
        """
        for state, state_group in self.test.groupby(by='state'):
            print('=====')
            print(state)
            print(state_group['target'].describe())
            print('=====')
        input()
        """
        results = []
        ...
        columns = []
        for state in states:
            mean = self.test[self.test['state']==state]['target'].mean()
            stddev = self.test[self.test['state']==state]['target'].std()
            num_trades = len(self.test[self.test['state']==state]['target'])
            if num_trades>0:
                accuracy = len(self.test[ (self.test['state']==state) & (self.test['target']>0) ]) / float(num_trades)
            else:
                accuracy = None

            #results['state_'+str(state)] = {'mean': mean, 'stddev': stddev, 'num_trades': num_trades, 'accuracy': accuracy}
            #results.extend( {'mean_'+str(state): mean, 'stddev_'+str(state): stddev, 'num_trades_'+str(state): num_trades, 'accuracy_'+str(state): accuracy} )
            results.extend([num_trades, accuracy, mean, stddev])
            columns.extend(['num_trades_'+str(state), 'accuracy_'+str(state),'mean_'+str(state), 'stddev_'+str(state)])

        
        results = pd.DataFrame([results], columns = columns)
        
        model_params = [str(self.features), self.hold_length, 
                        self.period, self.pattern, 
                        self.algo, self.num_features, self.max_depth, self.scoring, self.score]
        self.df = pd.DataFrame([model_params], columns = ['features', 'hold_length', 'period', 'pattern', 'algo', 'num_features', 'max_depth', 'scoring',  'score'])
        self.df['name'] = self.param_group_name
        print(results)
        results = pd.concat([results, self.df],axis=1)

        results.to_sql('model_summary', self.conn, if_exists='append')
        self.trades.to_sql('trades', self.conn, if_exists='append')
    ...
    def store_results(self):
        ...
        self.all_trades.to_sql('hidden_states_models_trades_%s' % self.sector, self.conn, if_exists='append', index=False)
        self.df.to_sql('hidden_states_models_%s' % self.sector, self.conn, if_exists='append', index=False)
        #self.total_result.to_sql('hidden_states_total_results', self.conn, if_exists='append', index=False)
        self.individual_results.to_sql('hidden_states_individual_results_%s' % self.sector, self.conn, if_exists='append', index=False)
    """        
...
if __name__ == '__main__':
    ...
    periods = ['2y','3y','4y','5y','10y','15y', 'max']
    #trade_other_states_list = [True, False]
    trade_time = ['open']
    ...
    print(tickers)
    benchmark = ['QQQ']
    inputs = list( product( periods, algos, hold_length, pattern, max_depth, trade_time, scoring, k_features, marketcaps ) )
    shuffle(inputs)
    for period, algo, hold_length, pattern, max_depth, trade_time, scoring, k_features, marketcap in list(inputs):
