...
class stock_predictor():
    ...
    def run_grid_search(self):
        ...
        param_grid = [{
           'sfs__k_features': list(self.k_features),
            'sfs__estimator__max_depth': list(self.max_depth) + [10,15,20,25]
        }]
        print(param_grid)
    ...
    def store_results(self):
        ...
        self.df.to_sql('model_summaries_%s' % self.sector, self.conn, if_exists='append', index=False)
...
if __name__ == '__main__':
    ...
    trade_other_states_list = [True, False]
    trade_time = ['today', 'tomorrow' ,'intraday']
    ...
    for i in all_inputs:
        ...
        input()
    """
    inputs = list( product ( periods, trade_other_states_list, trade_time, algos, hold_length, pattern, max_depth, scoring, k_features ) )
    shuffle(inputs)
    for period, trade_other_states, trade_time, algo, hold_length, pattern, max_depth, scoring, k_features in list(inputs):
